...
export const DEFAULT_SYNC_BOOTSTRAP_MODE: SyncBootstrapMode = "count";
...
export function resolveConfig() {
  ...
  const exportDir = process.env.CHATGPT_EXPORT_DIR?.trim() || DEFAULT_EXPORT_DIR;
  const syncMode = (process.env.CHATGPT_SYNC_MODE?.trim() || DEFAULT_SYNC_MODE) as SyncMode;
  const listLimit = parsePositiveInt(process.env.CHATGPT_LIST_LIMIT, DEFAULT_LIST_LIMIT);
  const syncCount = parsePositiveInt(
    process.env.CHATGPT_SYNC_COUNT ?? process.env.CHATGPT_EXPORT_LIMIT,
    DEFAULT_SYNC_COUNT,
  );
  const syncDays = parsePositiveInt(process.env.CHATGPT_SYNC_DAYS, DEFAULT_SYNC_DAYS);
  const syncOverlapMinutes = parsePositiveInt(
    process.env.CHATGPT_SYNC_OVERLAP_MINUTES,
    DEFAULT_SYNC_OVERLAP_MINUTES,
  );
  const bootstrapMode = (process.env.CHATGPT_SYNC_BOOTSTRAP_MODE?.trim() ||
    DEFAULT_SYNC_BOOTSTRAP_MODE) as SyncBootstrapMode;
